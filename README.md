...
Solução: O que estava ocorendo era que ao apagar um elemento par da lista o dataset ja apontava para o proximo elemento do mesmo, e logo em seguida dava um next para o proximo registro, fazendo assim pular um registro sempre que encontrava um elemento par. Para corrigi adicionei um continue para que não avançase para o proximo registro do dataset sem fazer a validação
...
Solução: O que estava ocorendo era um sobrecarregamento da memoria, para corrigir assim que é instanciado o objeto do clientDataSet coloquei um try/finaly para que quando terminasse de processar leberasse da memoria o clientDataSet, isso foi feito tanto no metodo de salvar o arquivo quando no metodo do botão "enviar sem erro"
